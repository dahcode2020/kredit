...
import { simulateCredit, SimulateInput, INCOME_TYPES, EMPLOYMENT_STATUSES, LOAN_PURPOSES } from "@/lib/credit-engine";
...
const products = [
  { key: "PERSONAL", rate: 0.0399, min: 1500, max: 50000, maxTerm: 84 },
  { key: "MORTGAGE", rate: 0.0325, min: 50000, max: 500000, maxTerm: 300 },
  { key: "BUSINESS", rate: 0.045, min: 5000, max: 250000, maxTerm: 120 },
] as const;
        ...
        <div className="lg:col-span-3 p-6 md:p-8 space-y-5">
          <div className="flex flex-wrap gap-2">
            {products.map((prod, i) => (
              <button key={prod.key} onClick={() => { setProductIdx(i); setAmount(Math.min(Math.max(prod.min, amount), prod.max)); setTerm(Math.min(term, prod.maxTerm)); }}
                ...
                {tr(`simulator.tab.${prod.key}`)} • {(prod.rate*100).toFixed(2)}% <span className="opacity-60">{tr("simulator.from")}</span>
              </button>
            ))}
          </div>
          ...
          {!hasError ? (
            <div className="grid grid-cols-3 gap-3 pt-2">
              <div className="bg-surface rounded-2xl p-4 border"><div className="text-[11px] tracking-widest uppercase font-bold text-slate-500">{tr("simulator.monthly")}</div><div className="text-[22px] font-extrabold text-ink">{eur(result.simulation.monthlyPayment)}</div><div className="text-[11px] text-slate-400">{tr("simulator.perMonth")}</div></div>
              <div className="bg-surface rounded-2xl p-4 border"><div className="text-[11px] tracking-widest uppercase font-bold text-slate-500">{tr("simulator.taeg")}</div><div className="text-[22px] font-extrabold text-ink">{(result.simulation.taeg*100).toFixed(2)}%</div><div className="text-[11px] text-slate-400">{tr("simulator.taegIndicative")}</div></div>
              <div className="bg-surface rounded-2xl p-4 border"><div className="text-[11px] tracking-widest uppercase font-bold text-slate-500">{tr("simulator.total")}</div><div className="text-[22px] font-extrabold text-ink">{eur(result.simulation.totalCost)}</div><div className="text-[11px] text-emerald-600">{tr("simulator.breakdown", { interest: eur(result.simulation.totalInterest), fees: eur(result.simulation.fees.total) })}</div></div>
            </div>
          ) : (
          ...
          {!hasError && (
            <div className="grid md:grid-cols-3 gap-3">
              ...
              <div className="bg-white rounded-2xl p-4 border"><div className="text-[11px] tracking-widest uppercase font-bold text-slate-500">{tr("eligibility.debtRatio")}</div><div className={`text-lg font-extrabold ${result.eligibility.debtRatio>0.33?'text-amber-600': 'text-emerald-600'}`}>{(result.eligibility.debtRatio*100).toFixed(1)}%</div><div className="text-xs text-slate-500">{tr("eligibility.capacity")} {eur(result.eligibility.repaymentCapacity)}</div></div>
              <div className="bg-white rounded-2xl p-4 border"><div className="text-[11px] tracking-widest uppercase font-bold text-slate-500">{tr("simulator.score")}</div><div className="text-lg font-extrabold text-ink">{result.score.value} <span className="text-sm font-bold px-2 py-1 rounded-full bg-ink text-white ml-1">{result.score.grade}</span></div><div className="text-xs text-slate-500">{trOu("simulator.scoreExplanation", result.score.explanation, { value: result.score.value, grade: result.score.grade, pct: `${(result.eligibility.debtRatio*100).toFixed(1)}%` })}</div></div>
            </div>
          )}
          ...
          {!hasError && (
            <div className="flex flex-wrap gap-2">
              ...
              <button className="inline-flex items-center gap-2 h-11 px-5 rounded-full border text-sm font-semibold"><FileText className="w-4 h-4"/> {tr("simulator.docsButton", { count: result.requiredDocuments.length })}</button>
            </div>
          )}
