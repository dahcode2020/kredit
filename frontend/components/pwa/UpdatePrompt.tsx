...
export default function UpdatePrompt() {
  ...
  return (
    <div role="alert" aria-live="assertive" className="fixed top-[76px] left-1/2 -translate-x-1/2 bg-ink text-white rounded-full shadow-card border border-white/10 px-4 py-2 flex items-center gap-3 z-50 max-w-[90vw]">
      ...
      <span className="text-sm font-semibold whitespace-nowrap">Mise à jour disponible</span>
      <Button size="md" className="!h-7 !px-3 text-xs gap-1.5 whitespace-nowrap" onClick={applyUpdate}><RefreshCw className="w-3.5 h-3.5"/> Actualiser</Button>
      <button onClick={onDismiss} aria-label="Ignorer la mise à jour" className="w-6 h-6 rounded-full bg-white/10 grid place-items-center hover:bg-white/20 shrink-0"><X className="w-3 h-3"/></button>
    </div>
  );
}
