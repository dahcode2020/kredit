...
export function formatDate(date: Date | string, locale: Locale, opts?: Intl.DateTimeFormatOptions) {
  ...
  return new Intl.DateTimeFormat(localeToIntl[locale], { day: "2-digit", month: "2-digit", year: "numeric", timeZone, ...opts }).format(d);
}
export function formatDateTime(date: Date | string, locale: Locale) {
  const d = typeof date === "string" ? new Date(date) : date;
  return new Intl.DateTimeFormat(localeToIntl[locale], { dateStyle: "medium", timeStyle: "short", timeZone }).format(d);
}
export function formatDateLong(date: Date | string, locale: Locale) {
  const d = typeof date === "string" ? new Date(date) : date;
  return new Intl.DateTimeFormat(localeToIntl[locale], { dateStyle: "long", timeZone }).format(d);
}
export function formatRelative(date: Date | string, locale: Locale) {
  ...
  if (Math.abs(days) < 1) {
    ...
    if (Math.abs(hours) < 1) return new Intl.RelativeTimeFormat(localeToIntl[locale], { numeric: "auto" }).format(-Math.round(diff / 60000), "minute");
    return new Intl.RelativeTimeFormat(localeToIntl[locale], { numeric: "auto" }).format(-hours, "hour");
  }
  return new Intl.RelativeTimeFormat(localeToIntl[locale], { numeric: "auto" }).format(-days, "day");
}

// Numbers
export function formatNumber(n: number, locale: Locale, opts?: Intl.NumberFormatOptions) {
  return new Intl.NumberFormat(localeToIntl[locale], opts).format(n);
}
export function formatCurrency(n: number, locale: Locale, currency: string = "EUR") {
  return new Intl.NumberFormat(localeToIntl[locale], { style: "currency", currency, minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(n);
}
export function formatCurrency0(n: number, locale: Locale, currency: string = "EUR") {
  return new Intl.NumberFormat(localeToIntl[locale], { style: "currency", currency, maximumFractionDigits: 0 }).format(n);
}
export function formatPercent(n: number, locale: Locale, digits = 1) {
  // n = 0.384 => 38.4%
  return new Intl.NumberFormat(localeToIntl[locale], { style: "percent", minimumFractionDigits: digits, maximumFractionDigits: digits }).format(n);
}
export function formatList(items: string[], locale: Locale, type: "conjunction" | "disjunction" = "conjunction") {
  return new Intl.ListFormat(localeToIntl[locale], { style: "long", type }).format(items);
}
