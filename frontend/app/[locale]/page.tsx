            ...
            <div className="relative">
              <div className="bg-white rounded-[24px] shadow-card p-6 md:p-7">
                ...
                <div className="mt-5 grid grid-cols-3 gap-3 text-center">
                  <div className="rounded-2xl bg-surface border p-3"><div className="text-[11px] font-bold tracking-widest uppercase text-slate-500">{tr("heroCard.from")}</div><div className="font-extrabold text-ink">3,25%</div><div className="text-[11px] text-slate-400">TAEG</div></div>
                  <div className="rounded-2xl bg-surface border p-3"><div className="text-[11px] font-bold tracking-widest uppercase text-slate-500">{tr("heroCard.upTo")}</div><div className="font-extrabold text-ink">500k€</div><div className="text-[11px] text-slate-400">{tr("heroCard.upToSub")}</div></div>
        ...
        <div className="grid lg:grid-cols-2 gap-10 items-start">
          <div>
            ...
            <ul className="mt-6 space-y-3">
              {["aboutCheck.1", "aboutCheck.2", "aboutCheck.3"].map(li => (
                <li key={li} className="flex gap-3 text-sm"><span className="mt-0.5 w-6 h-6 rounded-full bg-primary/10 text-primary grid place-items-center shrink-0"><Check className="w-3.5 h-3.5" /></span><span className="text-slate-700">{tr(li)}</span></li>
              ))}
            </ul>
      ...
      <section id="produits" className="bg-surface py-16 md:py-20">
        <div className="mx-auto max-w-[1280px] px-6">
          ...
          <div className="mt-10 grid md:grid-cols-3 lg:grid-cols-4 gap-6">
            {[
              { icon: Wallet, title: tr("products.personal"), descKey: "products.personal.desc", img: "https://images.unsplash.com/photo-1553729459-efe14ef6055d?w=600&auto=format&fit=crop&q=80", tagKey: "products.personal.tag" },
              { icon: Home, title: tr("products.mortgage"), descKey: "products.mortgage.desc", img: "https://images.unsplash.com/photo-1560518883-ce09059eeffa?w=600&auto=format&fit=crop&q=80", tagKey: "products.mortgage.tag" },
              { icon: Briefcase, title: tr("products.business"), descKey: "products.business.desc", img: "https://images.unsplash.com/photo-1556761175-5973dc0f32e7?w=600&auto=format&fit=crop&q=80", tagKey: "products.business.tag" },
              { icon: BarChart3, title: tr("products.invest"), descKey: "products.invest.desc", img: "https://images.unsplash.com/photo-1611974789855-9c2a0a7236a3?w=600&auto=format&fit=crop&q=80", tagKey: "products.invest.tag" },
            ].map(card => (
      ...
      <section className="mx-auto max-w-[1280px] px-6 py-16">
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-2">
          {[
            ...
            <button key={tab.t} onClick={() => setActiveTab(i)} className={`px-5 py-3 rounded-full text-xs font-bold tracking-widest uppercase border transition ${activeTab===i ? "bg-ink text-white border-ink" : "bg-white text-slate-600 border-slate-200 hover:border-ink/20"}`}>{tr(tab.t)}</button>
          ))}
        </div>
        ...
        <div className="mx-auto max-w-[1280px] px-6">
          ...
          <div className="mt-10 grid md:grid-cols-3 gap-6">
            {[
              ...
              <div key={s.titleKey} className="bg-white/[0.04] border border-white/10 rounded-[20px] p-6 hover:bg-white/[0.06] transition">
        ...
        <div className="relative mx-auto max-w-[1280px] px-6">
          ...
          <div className="mt-8 grid md:grid-cols-3 gap-6">
            {[
              ...
              <div key={tes.nKey} className="bg-white rounded-2xl p-6 shadow-card">
          ...
          {[
            ...
            <div key={card.role} className="bg-white rounded-[24px] border shadow-soft p-6">
              ...
              {already ? (
                ...
                <button onClick={() => handleDemoLogin(card.key)} className="mt-5 w-full h-11 rounded-full bg-white border border-slate-200 font-bold text-sm hover:bg-surface flex items-center justify-center gap-2">{tr(card.ctaKey)} <ArrowRight className="w-4 h-4"/></button>
              )}
              <div className="text-[11px] text-slate-400 text-center mt-2">{card.key === "CUSTOMER" ? "alex@kredit.be" : card.key === "ADMIN" ? "admin@kredit.be" : "super@kredit.be"} — {tr("roles.demoNote")}</div>
            </div>
          )})}
        </div>
